    ...
    <div className="space-y-5">
      ...
      */}
      ...
      {/* )} — fim do ternário SmartQuoteWizard */}
        ...
        {/* Right Column: Orçamento — hidden on mobile, shown on desktop */}
        <div className="hidden lg:flex clean-card p-4 rounded-2xl sticky top-4 max-h-[calc(100vh-2rem)] flex-col justify-between shadow-xl">
      ...
      {cart.length > 0 && (
        <button
          ...
          className="lg:hidden fixed bottom-6 right-5 z-40 flex items-center gap-2 px-4 py-3 bg-sky-600 hover:bg-sky-500 text-white font-bold text-sm rounded-2xl shadow-2xl transition-all active:scale-95"
        >
          <ShoppingBag className="w-5 h-5" />
          <span>Orçamento</span>
          <span className="ml-1 bg-white text-sky-600 text-xs font-black rounded-full w-5 h-5 flex items-center justify-center">
      ...
      {isCartSheetOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex flex-col justify-end">
          ...
          />
          {/* Sheet */}
          <div className="relative bg-white dark:bg-slate-900 rounded-t-3xl shadow-2xl flex flex-col max-h-[90vh] animate-slideUp">
            {/* Handle bar */}
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2">
                <ShoppingBag className="w-4 h-4 text-sky-600 dark:text-sky-400" />
                <h3 className="font-bold text-slate-900 dark:text-white text-sm font-outfit">Orçamento</h3>
                <span className="bg-sky-100 dark:bg-sky-900/40 text-sky-600 dark:text-sky-400 text-[10px] font-black rounded-full px-2 py-0.5">
              ...
              <div className="flex items-center gap-3">
                {cart.length > 0 && (
                  <>
                    <button onClick={clearCart} title="Limpar Orçamento" className="p-1.5 rounded-xl text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors">
                      <Trash2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => setShowSaveTemplateModal(true)} title="Salvar Kit" className="p-1.5 rounded-xl text-sky-600 dark:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-900/20 transition-colors">
                      <BookmarkPlus className="w-4 h-4" />
                    </button>
                  </>
                )}
                <button onClick={() => setIsCartSheetOpen(false)} className="p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            {/* Scrollable body — reuse the same desktop panel content */}
            <div className="overflow-y-auto flex-1 px-5 py-4 space-y-3.5">
              {/* Client Name */}
              <div>
                <label className="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Nome do Cliente</label>
                <input
                  type="text"
                  placeholder="ex: Carlos (Alfa Logística)"
                  value={clientName}
                  onChange={(e) => setClientName(e.target.value)}
                  className="w-full px-3 py-1.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs placeholder-slate-400 focus:outline-none focus:border-sky-500 font-medium"
                />
              </div>
              {/* Cart items */}
              <label className="block text-[11px] font-semibold text-slate-700 dark:text-slate-300">Produtos</label>
              <div className="space-y-2">
                {cart.map((item) => (
                  <div key={item.product.id} className="p-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-slate-900 dark:text-white truncate font-outfit">{item.product.name}</p>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        {editingItemPriceId === item.product.id ? (
                          ...
                          <div
                            ...
                            title="Toque para editar o preço"
                          >
                            <p className={`text-[11px] font-mono ${item.customPrice !== undefined ? 'text-sky-600 dark:text-sky-400 font-bold' : 'text-slate-500 dark:text-slate-400'}`}>
                              R$ {formatCurrency(item.customPrice !== undefined ? item.customPrice : item.product.default_price)}
                            </p>
                            <Edit3 className="w-3 h-3 text-slate-400 opacity-50 group-hover:opacity-100 transition-opacity" />
                          </div>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => updateQuantity(item.product.id, -1)} className="w-6 h-6 rounded-lg bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">
                        <Minus className="w-3 h-3" />
                      </button>
                      ...
                      <button onClick={() => toggleCartItem(item.product)} className="w-6 h-6 rounded-lg text-rose-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors flex items-center justify-center">
                        <X className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  </div>
              ...
              {cart.length > 0 && (
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1">
                  <div className="flex justify-between text-xs text-slate-500">
                    <span>Subtotal</span>
                    <span className="font-semibold">R$ {formatCurrency(rawEquipmentTotal)}</span>
                  </div>
                  {discountPercent > 0 && (
                    <div className="flex justify-between text-xs text-emerald-600">
                      <span>Desconto ({discountPercent}%)</span>
                      <span className="font-semibold">-R$ {formatCurrency(discountAmount)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm font-bold text-slate-900 dark:text-white pt-1">
                    <span>Total À Vista</span>
                    <span>R$ {formatCurrency(finalEquipmentPrice)}</span>
                  </div>
                  {includeMonthlyFee && totalMonthlyFee > 0 && (
                    <div className="flex justify-between text-xs text-emerald-600 dark:text-emerald-400">
                      <span>Mensalidade</span>
                      <span className="font-semibold">R$ {formatCurrency(totalMonthlyFee)}/mês</span>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="px-5 py-4 border-t border-slate-100 dark:border-slate-800 shrink-0">
              <button
                onClick={() => { handleCopyMessage(); }}
                disabled={cart.length === 0}
                className="w-full py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-300 dark:disabled:bg-slate-800 text-white font-bold text-sm shadow-md flex items-center justify-center gap-2 transition-all active:scale-95"
              >
                {copiedNotification ? (
                  <><CheckCircle className="w-4 h-4" /><span>Proposta Copiada!</span></>
                ) : (
                  <><Copy className="w-4 h-4" /><span>Copiar Mensagem WhatsApp</span></>
                )}
              </button>
            </div>
          </div>
